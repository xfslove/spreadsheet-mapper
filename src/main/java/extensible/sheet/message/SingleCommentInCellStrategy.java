package extensible.sheet.message;
...
import extensible.sheet.model.shapes.CommentBean;
import extensible.sheet.w2f.WorkbookWriteException;
...
public class SingleCommentInCellStrategy implements MessageWriteStrategy {
  ...
  public void write(OutputStream outputStream, Collection<ErrorMessage> errorMessages) {
    ...
    List<extensible.sheet.model.shapes.Comment> comments = transferToComments(errorMessages);

    for (extensible.sheet.model.shapes.Comment comment : comments) {
  ...
  private List<extensible.sheet.model.shapes.Comment> transferToComments(Collection<ErrorMessage> errorMessages) {
    ...
    List<extensible.sheet.model.shapes.Comment> comments = new ArrayList<>();
  ...
  private void addComment(Sheet sheet, extensible.sheet.model.shapes.Comment comment) {
